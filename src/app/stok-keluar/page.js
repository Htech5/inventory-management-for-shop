...
export default function StokKeluarPage() {
  ...
  const loadPageData = useCallback(async () => {
    ...
    try {
      const [itemsResponse, transactionsResponse] = await Promise.all([
        ...
        apiRequest("/api/stock/transactions?type=out"),
      ]);
      const items = itemsResponse.data.map(normalizeItem);

      setDaftarBarangMaster(items);
      setHistoryKeluar(transactionsResponse.data.map(normalizeTransaction));
  ...
  const handleSubmit = async (e) => {
    ...
    const barangDipilih = daftarBarangMaster.find(
      (barang) => barang.id === selectedBarang,
    );

    if (barangDipilih && barangDipilih.stok < Number(jumlahKeluar)) {
      alert(
        `Transaksi Ditolak! Stok "${barangDipilih.nama}" tidak mencukupi. (Sisa Stok: ${barangDipilih.stok})`,
      );
      return;
    }
            ...
            <button
              onClick={() => {
                if (daftarBarangMaster.length === 0) {
                  alert(
                    "Data master barang kosong! Silakan tambah data di dashboard halaman utama.",
                  );
                  return;
                }
                setIsOpen(true);
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          ...
          >
            ...
            <div className="bg-[#F9FAFB] p-4 px-6 flex justify-end gap-3 border-t border-gray-50">
              ...
              <button
                type="submit"
                className="h-[41.33px] px-5 rounded-xl text-sm font-semibold text-white bg-[#EF4444] hover:bg-red-600 shadow-md cursor-pointer"
              >
                Kurangi Stok
              </button>
            </div>
